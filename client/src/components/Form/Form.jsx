...
function validate(form) {
  ...
  if (form.name.length > 50) {
    ...
    errors.name =
      "Only letters, numbers, hypens(-), and parentheses are accepted";
  }
  if (!form.background_image) {
    errors.background_image = "Image is required";
  } else if (
    form.background_image.length !== 0 &&
    !/^(https?|chrome):\/\/[^\s$.?#].[^\s]*$/.test(form.background_image)
  ) {
...
const Form = () => {
  ...
  const submitHandler = (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      alert(
        "You need to fill all mandatory fields correctly before submitting"
      );
    } else {
      axios.post("/videogames", form).then(() => {
      ...
      <div className={s.form__page}>
        ...
        >
          ...
          <div>
            ...
            />
            {errors.released && (
              <p className={s.error__form}>{errors.released}</p>
            )}
          </div>
          <br></br>
          <div>
          ...
          <div>
            ...
            {errors.background_image && (
              <p className={s.error__form}>{errors.background_image}</p>
            )}
          </div>
          <br></br>
          <div>
            ...
            />
            {errors.description && (
              <p className={s.error__form}>{errors.description}</p>
            )}
          </div>
          <br></br>
          <div>
          ...
          <div className={s.form__selectedG}>
            ...
            {form.genres.map((e, i) => (
              <div className={s.options__label}>
                <div className={s.options__selectedG}>
                  {genres && genres.find((gen) => gen.id == e).name}
                </div>
                <button
                  key={i}
                  className={s.options__btn}
